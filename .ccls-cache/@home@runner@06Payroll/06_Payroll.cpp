...
string ceopass = "ceo1234", dirpass = "dir1234", promanpass, mangpass, asspass, sde1pass, sde2pass;
string ceousr = "ceouser", dirusr = "diruser", promanusr, mangusr, assusr, sde1usr, sde2usr;
// Name, department,\
...
void psdetails()
{
  cout << "Welcome to Sarkar's Infotech .Co\nWe welcome to the payroll interface(Plese give correct details to get accurate results) ";
...
void account()
{
  ...
  while (tries <= 3 && tries >= 0)
  {
    ...
    if (inputbyuser == ceousr || inputbyuser == dirusr || inputbyuser == promanusr || inputbyuser == assusr || inputbyuser == mangusr || inputbyuser == assusr || inputbyuser == sde1usr || inputbyuser == sde2usr)
    {
    ...
    else if (inputbyuser != ceousr || inputbyuser != dirusr || inputbyuser != promanusr || inputbyuser != assusr || inputbyuser != mangusr || inputbyuser != assusr || inputbyuser != sde1usr || inputbyuser != sde2usr)
    {
      cout << "you're wrong\n";
  ...
  for (tries1 = 0; tries1 <= 3 && tries1 >= 0; tries1)
  {
    ...
    cin >> inputbypassword;
    tries1++;
    if (inputbypassword == ceopass || inputbypassword == dirpass || inputbypassword == promanusr || inputbypassword == assusr || inputbypassword == mangusr || inputbypassword == assusr || inputbypassword == sde1usr || inputbypassword == sde2usr)
    {
      cout << "COngrat's you're correct\n";
    ...
    else if (inputbypassword != ceopass || inputbypassword != dirpass || inputbypassword != promanusr || inputbypassword != assusr || inputbypassword != mangusr || inputbypassword != assusr || inputbypassword != sde1usr || inputbypassword != sde2usr)
    {
      cout << "you're wrong\n";
...
{
  float ceosal = 120000, dirsal = 90000, promansal = 75000, mangsal = 45000, asssal = 30000, sde1sal = 80000, sde2sal = 100000;
